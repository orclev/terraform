...
resource "digitalocean_droplet" "nomad_master" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "docker run -d -p 8300 -p 8301 -p 8302 -p 8400 -p 8500 -p 8600 --name consul -v /etc/consul/consul_server.conf:/etc/consul/consul_server.conf voxxit/consul agent -config-file /etc/consul/consul_server.conf",
