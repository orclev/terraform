...
resource "digitalocean_droplet" "nomad_slave" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "docker run -d -p 8300:8300 -p 8301:8301 -p 8302:8302 -p 8400:8400 -p 8500:8500 -p 8600:8600 --name consul -v /etc/consul/consul_client.conf:/etc/consul/consul_client.conf voxxit/consul agent -config-file /etc/consul/consul_client.conf",
